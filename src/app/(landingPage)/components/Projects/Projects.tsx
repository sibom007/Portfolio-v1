...
import { useGetProjectsListQuery } from "@/Redux/api/ProjectsApi";
import LoaderPage from "@/components/Loader/Loader";
import Link from "next/link";
const Projects = () => {
  const { data, isFetching, isLoading } = useGetProjectsListQuery("");
            ...
            <TabPanel>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-white">
                {isFetching && isLoading ? (
                  ...
                  data?.result?.map((item: Tprojects, i: number) => (
                    <motion.div
                      initial={{ opacity: 0, y: 50 }}
                      whileInView={{ opacity: 1, y: 0 }}
                      transition={{
                      ...
                      <Image
                        ...
                        width={500}
                        height={400}
                        className="rounded-xl brightness-50 h-[390px]"
                      />
                      <div>
